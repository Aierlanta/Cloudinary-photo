...
export function setSecurityHeaders(response: Response): Response {
  ...
  response.headers.set(
    ...
    `default-src 'self'; img-src 'self' https://res.cloudinary.com https://api.telegram.org${tgStateDomain ? ` ${tgStateDomain}` : ''} data:; connect-src 'self' https://api.telegram.org https://res.cloudinary.com${tgStateDomain ? ` ${tgStateDomain}` : ''}; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; font-src 'self' data:;`
  );
